...
# Generation date: 2025-02-03T14:56:42
# Schema: finc
#
...
class Finc(YAMLRoot):
    ...
    class_model_uri: ClassVar[URIRef] = URIRef(
        "https://www.slub-dresden.de/linkml/finc/Finc"
    )
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if not isinstance(self.author_corporate, list):
            self.author_corporate = (
                [self.author_corporate] if self.author_corporate is not None else []
            )
        self.author_corporate = [
            v if isinstance(v, str) else str(v) for v in self.author_corporate
        ]

        if not isinstance(self.author_role, list):
            self.author_role = (
                [self.author_role] if self.author_role is not None else []
            )
        self.author_role = [
            v if isinstance(v, str) else str(v) for v in self.author_role
        ]

        if not isinstance(self.author2_role, list):
            self.author2_role = (
                [self.author2_role] if self.author2_role is not None else []
            )
        self.author2_role = [
            v if isinstance(v, str) else str(v) for v in self.author2_role
        ]

        if not isinstance(self.author_corporate_role, list):
            self.author_corporate_role = (
                [self.author_corporate_role]
                if self.author_corporate_role is not None
                else []
            )
        self.author_corporate_role = [
            v if isinstance(v, str) else str(v) for v in self.author_corporate_role
        ]
...
slots.finc__id = Slot(
    ...
    model_uri=DEFAULT_.finc__id,
    domain=None,
    range=URIRef,
)

slots.finc__record_id = Slot(
    uri=DEFAULT_.record_id,
    name="finc__record_id",
    curie=DEFAULT_.curie("record_id"),
    model_uri=DEFAULT_.finc__record_id,
    domain=None,
    range=str,
)

slots.finc__title = Slot(
    uri=DEFAULT_.title,
    name="finc__title",
    curie=DEFAULT_.curie("title"),
    model_uri=DEFAULT_.finc__title,
    domain=None,
    range=str,
)

slots.finc__topic = Slot(
    uri=DEFAULT_.topic,
    name="finc__topic",
    curie=DEFAULT_.curie("topic"),
    model_uri=DEFAULT_.finc__topic,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author = Slot(
    uri=DEFAULT_.author,
    name="finc__author",
    curie=DEFAULT_.curie("author"),
    model_uri=DEFAULT_.finc__author,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author2 = Slot(
    uri=DEFAULT_.author2,
    name="finc__author2",
    curie=DEFAULT_.curie("author2"),
    model_uri=DEFAULT_.finc__author2,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author_corporate = Slot(
    uri=DEFAULT_.author_corporate,
    name="finc__author_corporate",
    curie=DEFAULT_.curie("author_corporate"),
    model_uri=DEFAULT_.finc__author_corporate,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author_role = Slot(
    uri=DEFAULT_.author_role,
    name="finc__author_role",
    curie=DEFAULT_.curie("author_role"),
    model_uri=DEFAULT_.finc__author_role,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author2_role = Slot(
    uri=DEFAULT_.author2_role,
    name="finc__author2_role",
    curie=DEFAULT_.curie("author2_role"),
    model_uri=DEFAULT_.finc__author2_role,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author_corporate_role = Slot(
    uri=DEFAULT_.author_corporate_role,
    name="finc__author_corporate_role",
    curie=DEFAULT_.curie("author_corporate_role"),
    model_uri=DEFAULT_.finc__author_corporate_role,
    domain=None,
    range=Optional[Union[str, List[str]]],
)

slots.finc__author_sort = Slot(
    uri=DEFAULT_.author_sort,
    name="finc__author_sort",
    curie=DEFAULT_.curie("author_sort"),
    model_uri=DEFAULT_.finc__author_sort,
    domain=None,
    range=Optional[str],
)

slots.finc__isbn = Slot(
    uri=DEFAULT_.isbn,
    name="finc__isbn",
    curie=DEFAULT_.curie("isbn"),
    model_uri=DEFAULT_.finc__isbn,
    domain=None,
    range=Optional[str],
    pattern=re.compile(
